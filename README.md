...
默认运行 `API_MODE=mock`，不需要任何真实 API Key 即可完成本地 demo。切换 `API_MODE=live` 后，后端会尝试调用第三方 API；缺少 Key 或调用失败时会回退到 mock，避免演示中断。首个国内模型运行推荐使用 `API_PROVIDER_PRESET=china-qwen`，先让通义千问/百炼承担 LLM 对话和报告，ASR/TTS 保持 mock 兜底，后续再替换国内语音服务。
...
| 预设 | ASR | LLM | TTS | 用途 |
| --- | --- | --- | --- | --- |
| `china-qwen` | Mock 转写 | 通义千问/百炼 OpenAI 兼容接口 | Mock 播放状态 | 国内模型初步运行，优先验证对话、纠错和报告 |
| `global-mixed` | Deepgram | OpenAI | Cartesia | 海外混合语音栈，保留原演示路线 |
...
ASR/TTS 国内 provider 已在配置面板中预留 `aliyun-isi` 和 `iflytek` 选项。首版 live 代码只真实调用 Deepgram、OpenAI-compatible LLM 和 Cartesia；阿里云智能语音、讯飞语音识别/合成/评测因签名、音频格式和题型要求更复杂，当前显示为“已预留”，不会阻塞比赛 demo。
...
```bash
API_MODE=live
API_PROVIDER_PRESET=china-qwen
LLM_PROVIDER=qwen
...
也可以在 Web 端右上角“API 配置”中选择“国内初跑”，输入 LLM API Key 后保存。密钥只进入本地 Node API 的运行时内存，不写入浏览器持久化，也不会被 `/api/health` 或 `/api/settings` 返回。
