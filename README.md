...
- 实时链路：`ws` WebSocket 服务，路径为 `/api/practice/ws`，用于连续 practice session、VAD 断句事件和 AI 回复推送。
- 数据校验：Zod，用于接口结果和报告结构约束。
- 测试：Vitest、Supertest。
- UI：自定义 Fluent 2 风格 CSS、lucide-react 图标。
- 语音/模型 API 预留：三段式 `ASR -> LLM -> TTS`，默认展示比赛混合链路，并保留国内初跑预设。
...
4. 进入对话房间后点击“开始对话”，系统创建连续 `practice_session`。
5. 使用麦克风录音或本地演示输入，系统在停顿后自动断句，AI 根据回答动态追问并播报。
6. 点击“结束训练”，后端基于完整对话生成统一报告。
