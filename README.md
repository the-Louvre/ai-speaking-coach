# lingo coach - AI English Speaking Practice

lingo coach 是一款面向中文用户的 AI 英语口语陪练 Web 应用。它把“选择场景、连续开口、实时追问、纠错复盘、再次练习”做成一个完整闭环，帮助用户在面试、会议、点餐等真实场景里练习英语表达。

用户不需要按固定题库一轮轮答题。进入训练房间后，系统会创建 `practice_session`，在 3/5/7/10 分钟内持续记录对话；AI 根据用户刚刚说出的内容动态追问，训练结束后生成一页式课后报告，展示七维评分、句子纠错、表达优化、发音技巧和推荐重练句。

## Demo 视频

[大二学生3天做了个AI英语口语教练，效果到底怎么样？](https://www.bilibili.com/video/BV1ZHEh6PELr/?share_source=copy_web&vd_source=02e9b9605c9562efcf368306197ef715)

## 界面预览

### 首页：虚拟教练、每日目标与成长打卡

![首页：虚拟教练、每日目标与成长打卡](docs/demo-screenshots/home.png)

### 训练房间：连续对话、实时字幕与 AI 追问

![训练房间：连续对话、实时字幕与 AI 追问](docs/demo-screenshots/practice-room.png)

### 课后报告：七维评分、对话纠错与提升指导

![课后报告：七维评分、对话纠错与提升指导](docs/demo-screenshots/report.png)

## 题目需求覆盖

| 题目要求 | 本项目实现 |
| --- | --- |
| 场景选择 | 内置面试、会议、点餐场景，并支持自定义训练场景、AI 角色、任务目标和开场问题 |
| 实时语音对话 | 前端连接 Pipecat Voice Agent，支持连续训练房间、VAD 断句、AI 动态追问和 TTS 播放 |
| 发音评测 | 基于语音转写置信度、语速、低置信词、重复、停顿和教学 rubric 聚合发音维度反馈 |
| 语法/表达纠错 | LLM 基于本次对话生成原句与优化句对比，标注问题片段并给出中文解释 |
| 课后总结 | 训练结束后读取完整 `conversation_turns`，生成总分、七维评分、优势、问题和下次训练建议 |
| 自然度与延迟 | 训练从固定 5 轮问答改为 5-7 分钟连续对话，AI 根据上下文继续追问 |
| 可量化反馈 | 使用 fluency、pronunciation、grammar、vocabulary、coherence、task_completion、interaction 七维指标 |
| 学习持续性 | 首页提供连续打卡、火花状态、成长轨迹和下一轮训练目标 |

## 核心体验

- 低压力开口：中文用户不必先组织完美答案，先说出来，系统再帮助整理表达。
- 真实场景追问：AI 不按固定题库跳题，而是围绕当前目标持续追问，例如项目背景、个人职责、技术方案、结果和反思。
- 可解释纠错：报告展示用户原句、优化表达、问题片段、修改理由和更自然的说法。
- 发音与表达一起练：不仅指出语法问题，也给出重音、连读、弱读、语调和推荐重练句。
- 成长闭环：完成训练后点亮当天打卡，沉淀得分趋势、练习时长和下一步目标。

## 团队成员

- [the-Louvre](https://github.com/the-Louvre)：产品设计、前后端开发、UI 交互实现、API 接入与项目文档整理。
- [JoshuaZ16](https://github.com/JoshuaZ16)：前后端开发协作、功能优化、测试验证与 demo 演示录制。
...
- Demo 视频：已上传至 Bilibili，视频生成工程和大文件不纳入本项目本体提交
...
- 实时链路：Python Pipecat Voice Agent 服务，使用 SmallWebRTC 暴露 `/api/offer`，负责 VAD、STT、LLM Tutor、TTS 和音频输出。
- 业务会话：Node 后端维护 `practice_session`、`conversation_turns` 和最终报告，Pipecat 只把实时转写/AI 发言写回业务 API。
- 数据校验：Zod，用于接口结果和报告结构约束。
- 测试：Vitest、Supertest。
- UI：自定义 Fluent 2 风格 CSS、lucide-react 图标。
- 语音/模型 API：三段式 `ASR -> LLM -> TTS`，默认展示比赛混合链路，并保留国内初跑预设。

默认运行 `API_MODE=mock`，不需要任何真实 API Key 即可完成本地 demo。切换 `API_MODE=live` 后，后端会尝试调用第三方 API；缺少 Key 或调用失败时会回退到 mock，避免演示中断。当前比赛 live 配置推荐使用 `API_PROVIDER_PRESET=global-mixed`：AssemblyAI 负责 ASR，hezu OpenAI-compatible endpoint 负责 LLM，Cartesia 负责 TTS。`china-qwen` 仍作为可选国内模型预设保留。
...
| `china-qwen` | `qwen3-asr-flash` | 通义千问/百炼 OpenAI 兼容接口 | `qwen3-tts-flash` | 可选国内模型初步运行，验证转写、对话、纠错、报告和语音生成 |
...
- Pipecat：用于 WebRTC 实时语音 Agent 编排，连接浏览器音频输入、VAD、STT、LLM、TTS 与音频输出。
...
- Voice Agent：http://127.0.0.1:7860/health
- WebRTC Offer：http://127.0.0.1:7860/api/offer
...
- [Pipecat JS SmallWebRTCTransport](https://docs.pipecat.ai/api-reference/client/js/transports/small-webrtc)

### Demo 路线

1. 打开首页，查看虚拟教练欢迎卡和连续打卡状态。
2. 选择“面试 Interview”场景。
3. 进入任务准备页，选择 3/5/7/10 分钟训练时长。
4. 进入对话房间后点击“开始训练”，系统创建连续 `practice_session` 并连接 Pipecat Voice Agent。
5. 使用麦克风连续对话，Pipecat 根据 VAD 自动断句，AI Tutor 根据完整上下文动态追问并播报。
6. 点击“结束训练”关闭实时连接，再点击“生成报告”，后端基于完整对话生成统一报告。
7. 查看总分、七维评分雷达图、完整对话回放、错误修正、推荐重练任务和今日打卡完成状态。
