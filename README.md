...
ASR/TTS 国内 provider 已支持百炼 Qwen 初步调用，并在配置面板中继续预留 `aliyun-isi` 和 `iflytek` 选项。阿里云传统智能语音、讯飞语音识别/合成/评测因签名、音频格式和题型要求更复杂，当前仍作为后续增强，不阻塞比赛 demo。
...
ASR_API_KEY=your_local_key
TTS_PROVIDER=qwen-tts
TTS_MODEL=qwen3-tts-flash
TTS_API_KEY=your_local_key
TTS_VOICE_ID=Cherry
PRONUNCIATION_PROVIDER=qwen
```

也可以在 Web 端右上角“API 配置”中选择“国内初跑”，输入 ASR/LLM/TTS API Key 后保存。密钥只进入本地 Node API 的运行时内存，不写入浏览器持久化，也不会被 `/api/health` 或 `/api/settings` 返回。
